...
# 结论节点提示
if not exp_a:
    warns.append("本文未识别到结论（既无结语/余论/结论标题，末段也不以「综上…」开头）：A_level 为空，需人工补")
...
for b in B:
    pid = b.get("parent")
    if b.get("depth") == 2:
        # depth2 挂在唯一结论节点 A 下；A 缺失时允许 parent 为 None
        if pid is not None and pid not in a_ids:
            errs.append("%s(depth2) parent 应为 A，实际 %s" % (b["id"], pid))
    else:
        pb = got_b.get(pid)
        if pb is None:
            errs.append("%s(depth%s) parent 应为 depth%s 的 B，实际 %s"
                        % (b["id"], b.get("depth"), b.get("depth") - 1, pid))
        elif pb.get("depth") != b.get("depth") - 1:
            errs.append("%s(depth%s) parent %s 应为 depth%s，实际 depth%s"
                        % (b["id"], b.get("depth"), pid, b.get("depth") - 1, pb.get("depth")))
# span 嵌套
def span_of(node):
...
for b in B:
    if b.get("depth") == 2:
        # depth2 直接挂在唯一结论节点 A 下；A 是全文结论（span 仅覆盖结论段），
        # 不作 span 包含校验——各章位于结论段之前，空间上本就不被 A 的 span 包含
        continue
    pb = got_b.get(b.get("parent"))
    if pb:
...
for n in claim_nodes:
    ...
    if sup is None:
        ...
        if n["id"] in a_ids:
            # 结论节点是根，其下所有 B / C 均为后代
            desc |= b_ids | {c["id"] for c in C}
        for x in sby:
...
# id 全局唯一（防跨父撞号：如结语主张旧编号 C1.x 与第一章撞号）
ids_all = [x["id"] for x in A + B + C]
dup_ids = sorted({i for i in ids_all if ids_all.count(i) > 1})
if dup_ids:
    errs.append("id 全局重复（跨父撞号）：%s" % dup_ids)

# C 编号连续 + 同父内 anchor 页序非递减
chapter_count = max([int(m.group(1)) for m in
                     (re.match(r"B(\d+)", b["id"]) for b in B) if m] or [0])
for pid, cs in by_parent.items():
    nums = []
    # 结语主张（父为 A 层）续末章之后编号
    expect_prefix = ("C%d." % (chapter_count + 1)) if pid.startswith("A") else ("C" + pid[1:] + ".")
    for c in cs:
        tail = c["id"].split(".")[-1]
...
max_depth = max([1] + [b.get("depth", 2) for b in B])
nbd = {d: 0 for d in range(1, max_depth + 1)}
for b in B:
    nbd[b.get("depth")] = nbd.get(b.get("depth"), 0) + 1
nbd[1] = len(A)
want_nbd = {str(d): nbd[d] for d in range(1, max_depth + 1)}
if st.get("nodes_by_depth") != want_nbd:
    errs.append("statistics.nodes_by_depth=%s，实际 %s"
                % (st.get("nodes_by_depth"), want_nbd))
for a in A:
...
print("骨架: A=%d  B=%d %s  front_matter=%s"
      % (len(exp_a), len(exp_b),
         {("depth%d" % d): nbd.get(d, 0) for d in range(2, max_depth + 1)},
         sk["front_matter"]))
