...
from 公共库 import CN, RE_H1_NUM, RE_H2, RE_H3, RE_SPECIAL, RE_YEAR, heading_level
...
def build_S_index(source_pool):
    ...
    for s in source_pool["sources"]:
        for p in s["passages"]:
            ...
            m_num = re.match(r"(\d+)", oid[1:])  # 兼容 M77-b 这类带后缀编号
            materials.append({
                "id": mid,
                "text": p.get("text", ""),
                "source_full": p.get("source_full", ""),
                "footnote_number": int(m_num.group(1)) if m_num else 0,
                "src_passage_id": p["passage_id"],
            })
            src2m[p["passage_id"]] = mid
    # 按 M 编号排序
    materials.sort(key=lambda m: int(re.match(r"(\d+)", m["id"][1:]).group(1)))
...
def parse_paragraphs(md_text, src2m):
    ...
    def flush(buf):
        ...
        # 标题检测：与 公共库.heading_level 保持一致（结论类/一级/二级/三级）
        is_title = heading_level(first_line) is not None
        if is_title:
            title_text = first_line
        elif pid[0] == 1:
            # 首段不匹配任何层级标题 → 视为论文主标题（derive_skeleton 会自动跳过）
            is_title = True
            title_text = first_line
        ...
        if is_title:
            ...
            body = stripped
            # SRC_xxx_Pyy → original_id (M<数字>)；兼容模型省略 P 的写法 SRC_xxx_y
            def repl(m):
                sid = f"SRC_{m.group(1)}_P{m.group(2)}"
                return src2m.get(sid, m.group(0))
